...
if [ $gpu_id -gt -1 ]; then
    ...
    THEANO_FLAGS="cuda.root=/usr/local/cuda-10.1/bin,mode=FAST_RUN,device=cuda$gpu_id,floatX=float32,on_unused_input=ignore,dnn.include_path=/usr/local/cuda/include,dnn.library_path=/usr/local/cuda/lib64"
    export THEANO_FLAGS
